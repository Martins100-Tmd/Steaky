...
const JobComponent = () => {
  return (
    <div className="flex flex-col items-center sm:w-ninety mx-auto w-full">
      <Navbar />
      <h1 className="text-3xl text-center font-hev mb-10 p-4 rounded shadow">
        Open positions in all location
      </h1>

      <div className="flex flex-col items-center sm:w-ninety mx-auto w-nineFive">
        <h1 className="text-xl text-center font-hev uppercase my-5 border-b border-green-500 pb-2">
          Engineering Roles
        </h1>
        <div className="grid sm:grid-cols-3 grid-cols-2 w-full gap-5">
          {Eng.map((item, index) => {
            return (
              <div
                className="bg-white p-5 rounded border-l-2 border-green-600 flex flex-col items-start duration-700 hover:scale-105 shadow"
                key={index}
              >
                <p className="text-xl text-start font-hev font-semibold mb-3">
                  {item.name}
                </p>
                <div className="flex flex-row items-center">
                  ...
                  <span className="text-sm font-hev text-gray-500">
                    {item.place}
                  </span>
                </div>
              </div>
